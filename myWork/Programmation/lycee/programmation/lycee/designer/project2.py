...
def pushButton_click():
    if checkInputs():
      ...
      if windows.comboBox.currentText() == "Puissance":
        windows.resultat.setText(str(puissance(A, B)))
      elif windows.comboBox.currentText() == "PGCD":
        windows.resultat.setText(str(pgcd(A, B)))
      elif windows.comboBox.currentText() == "PPCM":
...
def checkInputs():
  ...
  test = True
  if not (A.isdecimal() and B.isdecimal() and int(A) > 0 and int(B) > 0):
    test = False
...
app = QApplication([])
windows = loadUi ("C:/Users/ousse/Documents/lycee/programmation/lycee/designer/project2.ui")
windows.show()
